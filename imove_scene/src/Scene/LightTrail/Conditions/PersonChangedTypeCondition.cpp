...
int PersonChangedTypeCondition::check(float dt, std::vector<Action*> &actions) {
    ...
    lightPeople->for_each([&](std::shared_ptr<LightPerson> person){
        ...
        if((oldType.count(person->getId()) == 0 || oldType[person->getId()] != scene_interface::Bystander) && person->type == scene_interface::Bystander){
        ...
        else if((oldType.count(person->getId()) == 0 || (oldType[person->getId()] != scene_interface::Participant || oldType[person->getId()] != scene_interface::StandingStill)) && (person->type == scene_interface::Participant || person->type == scene_interface::StandingStill))
        {
