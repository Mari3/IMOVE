...
struct MixingFailsScenario : public Scenario {
    ...
    MixingFailsScenario(const LightTrailConfiguration& config): timer(10.f){
        ...
        people.push_back(Person(Location(0,y),Person::PersonType::Participant));
        people.push_back(Person(Location(config.screenWidth(),y),Person::PersonType::Participant));
    }
...
struct ManyScenario : public Scenario {
    ...
    ManyScenario(const LightTrailConfiguration &config){
        ...
        for(int i=0;i<10;++i){
            people.push_back(Person(Location(xRange.drawRandom(),yRange.drawRandom()),Person::PersonType::Participant));
        }
    }
...
struct FirstPersonScenario : public Scenario {
    ...
    void update(float dt) override {
        if(!timerDone){
            if(timer.update(dt)){
                people.push_back(Person(Location(0,sh/2.f),Person::PersonType::Participant));
                timerDone = true;
                timer.restart(20.f);
            }
        }else{
            ...
            if(!timer2Done && timer.update(dt)){
                people.push_back(Person(Location(50,sh/2.f),StandingStill));
                timer2Done = true;
            }
        }
    }
...
struct RestartAlternatingScenario : public Scenario {
    ...
    void update(float dt) override {
        if(!timerDone){
            ...
            people[0].setLocation(people[0].getLocation()+Vector2(20*dt,0));
            if(timer.update(dt)){
                people[0].type = Person::PersonType::None;
                timer2Done = true;
            }
        }
    }
...
struct BystanderScenario : public Scenario {
    ...
    BystanderScenario(const LightTrailConfiguration &config, Scene *scene) {
        for(int i=0;i<300;++i)
            scene->update(.1f);
        people.push_back(Person(Location(config.screenWidth()/2,config.screenHeight()+100),Person::PersonType::Bystander));
        people.push_back(Person(Location(config.screenWidth()/4,config.screenHeight()/2),StandingStill));
    }
...
struct ColorHoleScenario : public Scenario {
    ...
    ColorHoleScenario(const LightTrailConfiguration& config) : timer(20.f) {
        ...
        thresh = 340.f;
        people.push_back(Person(Location(config.screenWidth()/2,20),Person::PersonType::Participant));
        people.push_back(Person(Location(0,y),Person::PersonType::Participant));
        people.push_back(Person(Location(config.screenWidth(),y),Person::PersonType::Participant));
    }
